...
(defn describe-path [[dest direction thing]]
  (str "there is a " thing " going " direction " from here."))

(defn describe-paths [location edges]
  (str/join " " (map describe-path (edges location))))
...
(defn describe-objects [loc objs obj-locs]
  (let [describe-obj #(str "you see a " % " on the floor.")]
    (str/join " " (map describe-obj (objects-at loc objs obj-locs)))))

(defn look []
  (str/join " " [(describe-location @location nodes)
...
(defn inventory []
  (str/join " " (cons "ITEMS:" (objects-at "body" objects @object-locations))))
